r"""Voer Beikes review van de veranda-hoekfix-kandidaten (tuinscenes) door.

Leest de '## Review'-regels van de Werkbank-job 'Veranda-hoekfix: kandidaten tuinscenes' (goed | fix | weg | open) en:
  goed  -> nieuwe render vervangt ALLE_FINALS/blokhutten-tuinscenes/<scene>.png; de oude gaat naar
           ALLE_FINALS/_pre_hoekfix_backup/<scene>.png (bestaat die al, dan <scene>_<datum>.png)
...
Standaard dry-run; met --doen wordt er echt gekopieerd/verplaatst. Nooit committen: dat is Beikes stap.
  ...
  python scripts/apply_hoekfix_keuze.py [--doen]
"""
import os
import re
import shutil
...
JOB = r"C:\Users\beike\Desktop\Icloud\iCloudDrive\Vault\Claude\Werkbank\jobs\veranda-hoekfix-kandidaten-tuinscenes-6.md"
NIEUW = os.path.join(ROOT, "ALLE_FINALS", "hoekfix-kandidaten-2sep")
FINALS = os.path.join(ROOT, "ALLE_FINALS", "blokhutten-tuinscenes")
BACKUP = os.path.join(ROOT, "ALLE_FINALS", "_pre_hoekfix_backup")
R4 = {"zonnebloem_zonnegroet", "lavendel_buitenbioscoop", "roosmarijn_uitslaapochtend", "roosmarijn_schaakavond",
      "jasmijn_vinylmiddag", "jasmijn_vlindertuin"}
...
if not keuzes and "--markeer" not in sys.argv:
    print("Geen reviewregels voor de 6 hoekfix-kandidaten gevonden in de job; niets te doen.")
    sys.exit(0)
